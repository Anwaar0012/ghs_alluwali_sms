# views.py
from .models import Student, PrimaryResult, MiddleResult, HighResult,LEVEL_CHOICES,CLASS_CHOICES,RESULT_TYPE_CHOICES,AcademicYear
...
def result_detail(request, level, classs, roll_no,result_type):
    ...
    if ResultModel and result_type:
        results = ResultModel.objects.filter(student=student, result_type=result_type).order_by('-academic_year')
    elif ResultModel:
        results = ResultModel.objects.filter(student=student).order_by('-academic_year', 'result_type')
...
def class_results(request):
    ...
    if all([level, classs, result_type, year]):
        try:
            # Convert year to AcademicYear instance
            academic_year = AcademicYear.objects.get(year=int(year))
            ...
            if ResultModel:
                # Get results for these students in current academic year
    ...
    if export_format and students.exists():
        ...
        if export_format == 'excel':
            ...
            return export_pdf(headers, data)# ... keep your existing export code ... 
    ...
    context = {
        ...
        'LEVEL_CHOICES': Student._meta.get_field('level').choices,
        'CLASS_CHOICES': Student._meta.get_field('classs').choices,
        'RESULT_TYPE_CHOICES': MODEL_MAP.get(level, PrimaryResult)._meta.get_field('result_type').choices,
    }
